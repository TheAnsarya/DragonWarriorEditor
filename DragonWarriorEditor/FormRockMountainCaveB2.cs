...
{
    public partial class FormRockMountainCaveB2 : Form
    {
        string path;

        public FormRockMountainCaveB2(string gamePath)
        {
            InitializeComponent();
        ...
        */
        private void loadMap()
        {
            ...
            string rockMountainB2HexData = classDWBackend.getRockMountainB2Data();
            int x = 1;
            bool hasError = false;

            foreach (char ch in rockMountainB2HexData)
            {
                try
                {
                    PictureBox currentPictureBox = (PictureBox)FormRockMountainCaveB2.ActiveForm.Controls.Find("pictureBox" + x, true)[0];
        ...
        {
            ClassDWBackend classDWBackend = new ClassDWBackend(path);
            string newRockMountainB2HexData = "";
            for (int x = 1; x <= 196; x++)
            {
                try
                {
            ...
            bool result = classDWBackend.setRockMountainB2Data(newRockMountainB2HexData);
            if (result)
            {
