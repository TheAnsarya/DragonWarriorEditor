...
{
    public partial class FormErdricksCaveB1 : Form
    {
        string path;

        public FormErdricksCaveB1(string gamePath)
        {
            InitializeComponent();
        ...
        private void loadMap()
        {
            ...
            string erdricksCaveB1HexData = classDWBackend.getErdricksCaveB1Data();
            int x = 1;
            bool hasError = false;

            foreach (char ch in erdricksCaveB1HexData)
            {
                try
                {
                    PictureBox currentPictureBox = (PictureBox)FormErdricksCaveB1.ActiveForm.Controls.Find("pictureBox" + x, true)[0];
        ...
        {
            ClassDWBackend classDWBackend = new ClassDWBackend(path);
            string newErdricksCaveB1HexData = "";
            for (int x = 1; x <= 100; x++)
            {
                try
                {
            ...
            bool result = classDWBackend.setErdricksCaveB1Data(newErdricksCaveB1HexData);
            if (result)
            {
