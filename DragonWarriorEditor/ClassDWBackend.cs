...
namespace DragonWarriorEditor {
	class ClassDWBackend {
		...
		public string GetMapData(Definition.Map map) {
			MapInfo info = Definition.Maps[map];
			return getHexStringFromFile(info.Address, info.DataLength);
		}

		public bool SetMapData(Definition.Map map, string data) {
			return writeByteArrayToFile(this.path, Definition.Maps[map].Address, StringToByteArray(data));
		}
