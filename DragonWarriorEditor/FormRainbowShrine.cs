...
{
    public partial class FormRainbowShrine : Form
    {
        string path;

        public FormRainbowShrine(string gamePath)
        {
            InitializeComponent();
        ...
        private void loadMap()
        {
            ...
            string rainbowShrineHexData = classDWBackend.getRainbowShrineData();

            int x = 1;
            bool hasError = false;

            foreach (char ch in rainbowShrineHexData)
            {
                try
                {
                    PictureBox currentPictureBox = (PictureBox)FormRainbowShrine.ActiveForm.Controls.Find("pictureBox" + x, true)[0];
        ...
        private void saveMap()
        {
            ...
            bool result = classDWBackend.setRainbowShrineData(rainbowShrineHexData);
            if (result)
            {
