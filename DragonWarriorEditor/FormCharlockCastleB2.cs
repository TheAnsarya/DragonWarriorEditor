...
namespace DragonWarriorEditor {
	public partial class FormCharlockCastleB2 : Form {
		string path;

		public FormCharlockCastleB2(string gamePath) {
			InitializeComponent();
		...
		private void loadMap() {
			...
			string data = dataHandler.GetMapData(Definition.Map.CharlockCastleB2);
			...
			foreach (char ch in data) {
				try {
					PictureBox currentPictureBox = (PictureBox)FormCharlockCastleB2.ActiveForm.Controls.Find("pictureBox" + x, true)[0];
		...
		private void saveMap() {
			...
			bool result = classDWBackend.setCharlockCastleB2Data(newCharlockCastleB2HexData);
			if (result) {
