...
namespace DragonWarriorEditor {
	public partial class FormHauksnessRuins : Form {
		string path;

		public FormHauksnessRuins(string gamePath) {
			InitializeComponent();
		...
		private void loadMap() {
			...
			string data = dataHandler.GetMapData(Definition.Map.Hauksness);
			...
			foreach (char ch in data) {
				try {
					PictureBox currentPictureBox = (PictureBox)FormHauksnessRuins.ActiveForm.Controls.Find("pictureBox" + x, true)[0];
		...
		private void saveMap() {
			...
			bool result = classDWBackend.setHauksnessData(newHauksnessRuinsHexData);
			if (result) {
