...
{
    public partial class FormCharlockCastleB6 : Form
    {
        string path;

        public FormCharlockCastleB6(string gamePath)
        {
            InitializeComponent();
        ...
        private void loadMap()
        {
            ...
            string charlockCastleB6HexData = classDWBackend.getCharlockCastleB6Data();
            int x = 1;
            bool hasError = false;

            foreach (char ch in charlockCastleB6HexData)
            {
                try
                {
                    PictureBox currentPictureBox = (PictureBox)FormCharlockCastleB6.ActiveForm.Controls.Find("pictureBox" + x, true)[0];
        ...
        {
            DataHandler classDWBackend = new DataHandler(path);
            string newCharlockCastleB6HexData = "";
            for (int x = 1; x <= 100; x++)
            {
                try
                {
            ...
            bool result = classDWBackend.setCharlockCastleB6Data(newCharlockCastleB6HexData);
            if (result)
            {
