...
{
    public partial class FormCharlockCastleB3 : Form
    {
        string path;

        public FormCharlockCastleB3(string gamePath)
        {
            InitializeComponent();
        ...
        private void loadMap()
        {
            ...
            string charlockCastleB3HexData = classDWBackend.getCharlockCastleB3Data();
            int x = 1;
            bool hasError = false;

            foreach (char ch in charlockCastleB3HexData)
            {
                try
                {
                    PictureBox currentPictureBox = (PictureBox)FormCharlockCastleB3.ActiveForm.Controls.Find("pictureBox" + x, true)[0];
        ...
        {
            ClassDWBackend classDWBackend = new ClassDWBackend(path);
            string newCharlockCastleB3HexData = "";
            for (int x = 1; x <= 100; x++)
            {
                try
                {
            ...
            bool result = classDWBackend.setCharlockCastleB3Data(newCharlockCastleB3HexData);
            if (result)
            {
