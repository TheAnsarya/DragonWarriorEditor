...
{
    public partial class FormCharlockCastleB5 : Form
    {
        string path;

        public FormCharlockCastleB5(string gamePath)
        {
            InitializeComponent();
        ...
        private void loadMap()
        {
            ...
            string charlockCastleB5HexData = classDWBackend.getCharlockCastleB5Data();
            int x = 1;
            bool hasError = false;

            foreach (char ch in charlockCastleB5HexData)
            {
                try
                {
                    PictureBox currentPictureBox = (PictureBox)FormCharlockCastleB5.ActiveForm.Controls.Find("pictureBox" + x, true)[0];
        ...
        {
            DataHandler classDWBackend = new DataHandler(path);
            string newCharlockCastleB5HexData = "";
            for (int x = 1; x <= 100; x++)
            {
                try
                {
            ...
            bool result = classDWBackend.setCharlockCastleB5Data(newCharlockCastleB5HexData);
            if (result)
            {
