...
{
    public partial class FormCharlockCastleB4 : Form
    {
        string path;

        public FormCharlockCastleB4(string gamePath)
        {
            InitializeComponent();
        ...
        private void loadMap()
        {
            ...
            string charlockCastleB4HexData = classDWBackend.getCharlockCastleB4Data();
            int x = 1;
            bool hasError = false;

            foreach (char ch in charlockCastleB4HexData)
            {
                try
                {
                    PictureBox currentPictureBox = (PictureBox)FormCharlockCastleB4.ActiveForm.Controls.Find("pictureBox" + x, true)[0];
        ...
        {
            ClassDWBackend classDWBackend = new ClassDWBackend(path);
            string newCharlockCastleB4HexData = "";
            for (int x = 1; x <= 100; x++)
            {
                try
                {
            ...
            bool result = classDWBackend.setCharlockCastleB4Data(newCharlockCastleB4HexData);
            if (result)
            {
