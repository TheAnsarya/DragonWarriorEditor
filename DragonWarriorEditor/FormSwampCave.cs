...
{
    public partial class FormSwampCave : Form
    {
        string path;

        public FormSwampCave(string gamePath)
        {
            InitializeComponent();
        ...
        private void loadMap()
        {
            ...
            string swampCaveB2HexData = classDWBackend.getSwampCaveData();
            int x = 1;
            bool hasError = false;

            foreach (char ch in swampCaveB2HexData)
            {
                try
                {
                    PictureBox currentPictureBox = (PictureBox)FormSwampCave.ActiveForm.Controls.Find("pictureBox" + x, true)[0];
        ...
        private void saveMap()
        {
            ...
            bool result = classDWBackend.setSwampCaveData(newSwampCaveB2HexData);
            if (result)
            {
