...
namespace DragonWarriorEditor {
	public partial class FormRimuldar : Form {
		...
		public FormRimuldar(string gamePath) {
			InitializeComponent();
		...
		private void loadMap() {
			...
			string data = dataHandler.GetMapData(Definition.Map.Rimuldar);
			...
			foreach (char ch in data) {
				try {
					PictureBox currentPictureBox = (PictureBox)FormRimuldar.ActiveForm.Controls.Find("pictureBox" + x, true)[0];
		...
		private void saveMap() {
			...
			bool result = classDWBackend.setRimuldarData(newRimuldarHexData);
			if (result) {
