...
namespace DragonWarriorEditor {
	public partial class FormGarinham : Form {
		...
		public FormGarinham(string gamePath) {
			InitializeComponent();
		...
		private void loadMap() {
			...
			string data = dataHandler.GetMapData(Definition.Map.Garinham);
			...
			foreach (char ch in data) {
				try {
					PictureBox currentPictureBox = (PictureBox)FormGarinham.ActiveForm.Controls.Find("pictureBox" + x, true)[0];
		...
		private void saveMap() {
			...
			bool result = classDWBackend.setGarinhamData(newGarinhamHexData);
			if (result) {
